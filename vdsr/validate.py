...
def process_images(model,results_dir,flag='unpruned'):
    total_psnr = 0.0

    file_names = os.listdir(config.hr_dir)
    total_files = len(file_names)

    for index in range(total_files):
        sr_image_path = os.path.join(results_dir, flag, file_names[index])
    ...
    print(f"PSNR of {flag} model: {total_psnr / total_files:4.2f}dB.\n")

# Create a folder of super-resolution experiment results
results_dir = os.path.join("results", "test", config.exp_name)
if not os.path.exists(results_dir):
    os.makedirs(results_dir)
    os.makedirs(os.path.join(results_dir,"unpruned"))
...
unpruned_model = VDSR().to(device)
print("Built unpruned VDSR model.")
unpruned_checkpoint = torch.load(unpruned_model_path, map_location=lambda storage, loc: storage)
unpruned_model.load_state_dict(unpruned_checkpoint["state_dict"])
...
process_images(pruned_model, results_dir, 'pruned')
